...
const AUTHOR_IMAGES = [teamGregory, teamBenjamin];
...
export default function MethodologyAuthorsSection() {
  ...
  const authors = useMemo(() => {
    const items = t("methodology:authors.items", { returnObjects: true });
    return Array.isArray(items)
      ? items.map((author, index) => ({ ...author, image: AUTHOR_IMAGES[index] }))
      : [];
  }, [t]);
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-stretch lg:gap-12">
          <div className="flex flex-col gap-4 lg:max-w-[468px] lg:shrink-0 lg:gap-5">
            <h2 className="text-[40px] font-normal leading-[46px] text-[#201463] lg:text-[56px] lg:leading-[58px]">
              <span className="font-bold">{t("methodology:authors.headingBold")}</span>
              <span className="text-[rgba(32,20,99,0.7)]">{t("methodology:authors.headingMuted")}</span>
            </h2>
            <p className="text-base leading-6 text-[#4e546c]">{t("methodology:authors.intro")}</p>
          </div>
          ...
          <div className="flex flex-col items-center gap-6 lg:hidden">
            ...
            >
              {authors.map((author) => (
                <AuthorCard key={author.name} author={author} className="w-[340px] snap-center" />
              ))}
            </div>
            ...
            <div className="flex items-center gap-6">
              <button
                ...
                aria-label={t("methodology:authors.prev")}
              >
                <ArrowRightIcon className="h-4 w-4 rotate-180" dark />
              </button>

              <div className="flex items-center gap-2" role="tablist" aria-label={t("methodology:authors.headingMuted")}>
                {authors.map((author, i) => (
                  <button
                    ...
                    aria-label={t("methodology:authors.viewAuthor", { name: author.name })}
              ...
              <button
                ...
                className="flex size-8 items-center justify-center rounded-2xl border border-black transition enabled:hover:bg-black/5 disabled:opacity-40"
                aria-label={t("methodology:authors.next")}
              >
        ...
        <div className="flex w-full flex-col items-center gap-6 rounded-[20px] bg-[#12174b] px-5 py-12 lg:gap-6 lg:px-16 lg:py-16">
          <div className="flex w-full max-w-[1100px] flex-col gap-4 text-center lg:px-[150px]">
            <h2 className="text-[40px] font-normal leading-[46px] text-[#f2f6fb] lg:text-[52px] lg:leading-[58px]">
              <span className="font-bold">{t("methodology:authors.cta.headingBold")}</span>
              <span className="text-[rgba(242,246,251,0.7)]">
                {t("methodology:authors.cta.headingMuted")}
              </span>
            </h2>
            <p className="text-base leading-6 text-[rgba(242,246,251,0.75)]">
              {t("methodology:authors.cta.body")}
            </p>
          </div>
          ...
          <div className="flex w-full max-w-[600px] flex-col items-stretch gap-3 lg:max-w-none lg:flex-row lg:items-center lg:justify-center lg:gap-3">
            ...
            >
              ...
              {t("methodology:authors.cta.downloadReport")}
            </a>
